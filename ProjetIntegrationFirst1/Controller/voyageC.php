...
class voyageC{
    public function getVoyageByEtat2($etat){
        $sql = "SELECT * FROM voyage WHERE etat=:etat";
    ...
    public function switchEtat($id_voy,$etat){
        $sql = "UPDATE voyage SET etat=:etat  WHERE id_voy = :id_voy";
        $db = config::getConnexion();
        try {
            $query = $db->prepare($sql);
    ...
    public function getVoyageByEtat1($etat){
        $sql = "SELECT * FROM voyage WHERE etat=:etat";
        $db = config::getConnexion();
        try {
            $query = $db->prepare($sql);
    ...
    $tableHTML .= '<tr>';
        ...
        $tableHTML .= '<td><button class="btn btn-primary btn-update" data-id="' . $row['id_voy'] .'">Restaurer</button></td>';
    ...
    public function getVoyageByEtat($etat){
        $sql = "SELECT * FROM voyage WHERE etat=:etat";
        $db = config::getConnexion();
        try {
            $query = $db->prepare($sql);
    ...
    $tableHTML .= '<tr>';
        ...
        public function deleteVoyage($id_voy) {
            ...
            try {
                ...
                $query->execute();
                if ($query->rowCount() > 0) {
                    $this->logOperation("DELETE", $id_voy); // Log the "DELETE" operation
                }
    ...
    public function addVoyage($id_voy, $titre, $id_des, $date_debut, $date_fin, $prix, $description, $motivation, $moyen_transport, $etat) {
        $conn = config::getConnexion();
        
        try {
            $query = $conn->prepare("INSERT INTO voyage (id_voy, titre, id_des, date_debut, date_fin, prix, description, motivation, moyen_transport , etat) VALUES (:id_voy, :titre, :id_des, :date_debut, :date_fin, :prix, :description, :motivation, :moyen_transport ,:etat)");
            // Bind parameters
            $query->bindParam(':id_voy', $id_voy);
            ...
            if ($query->rowCount() > 0) {
                ...
                $this->logOperation("ADD", $id_voy); // Log the "ADD" operation
            }
        ...
        public function updateVoyage($id_voy,$titre,$id_des,$date_debut,$date_fin,$prix,$description,$motivation,$moyen_transport) {
            ...
            try {
                ...
                if ($query->rowCount() > 0) {
                    $this->logOperation("UPDATE", $id_voy); // Log the "UPDATE" operation
                }
